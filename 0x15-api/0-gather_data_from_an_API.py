...
if __name__ == "__main__":
    ...
    response_user = requests.get(api_url_user + '/{}'.format(sys.argv[1])).json()
    response_todo = requests.get(api_url_todo, param_todo).json()
    completed = [task.get('title') for task in response_todo if task.get('completed') is True]

    print("Employee {} is done with tasks ({}/{})".format(response_user.get('name'), len(completed), len(response_todo)))
